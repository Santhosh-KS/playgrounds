...
function M.validate()
    ...
    for lang, config in pairs(M.current.languages) do
        if config.enabled then
            ...
            if not exists then
                vim.notify(
                    string.format(
                        "Playground: %s executable '%s' not found. Disabling %s playground.",
                        lang, cmd, lang
                    ),
                    vim.log.levels.WARN
                )
    ...
    if M.current.split_position ~= 'right' and M.current.split_position ~= 'left' then
        vim.notify(
            "Invalid split_position value. Using 'right'.",
            vim.log.levels.WARN
        )
    ...
    if type(M.current.split_width) ~= 'number' or 
       M.current.split_width <= 0 or 
       M.current.split_width >= 1 then
        vim.notify(
            "Invalid split_width value. Using 0.5.",
            vim.log.levels.WARN
        )
